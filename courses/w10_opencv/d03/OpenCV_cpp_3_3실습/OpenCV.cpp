...
int main(void) {
    ...
    for ( i = 0 ; i < roadColor.cols ; i++) {
	    for ( j = 0 ; j < roadColor.rows ; j++) {
	        setPixel(channels[1], i, j, 255);
	    }
	}
	
	mergeImage(channels, HLS);
	Mat BGR;
	convertColor(HLS, BGR, COLOR_HSV2BGR);
