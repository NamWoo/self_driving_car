...
Mat imageProcessing(Mat &image) {
    ...
    vector<Point> ptr_l, ptr_r;
    ptr_l.push_back(Point(int(width * 0.45), int(height * 0.6)));
    ptr_l.push_back(Point(int(width * 0.5), int(height * 0.6)));
    ptr_l.push_back(Point(int(width * 0.5), int(height)));
    ptr_l.push_back(Point(int(width * 0.05), int(height)));
	polyROI(result, ROI_L, ptr_l);
    
    ptr_r.push_back(Point(int(width * 0.5), int(height * 0.6)));
    ptr_r.push_back(Point(int(width * 0.55), int(height * 0.6)));
    ptr_r.push_back(Point(int(width * 0.95), int(height)));
    ptr_r.push_back(Point(int(width * 0.5), int(height)));
	polyROI(result, ROI_R, ptr_r);
    ...
    Mat L_HSV, R_HSV;
    convertColor(ROI_L, L_HSV, COLOR_BGR2HSV);
    convertColor(ROI_R, R_HSV, COLOR_BGR2HSV);
    
    Mat L_Y, L_W, R_Y, R_W;
    rangeColor(L_HSV, L_W, lower_white_hsv, upper_white_hsv);
    rangeColor(L_HSV, L_Y, lower_yellow_hsv, upper_yellow_hsv);
    
    rangeColor(R_HSV, R_W, lower_white_hsv, upper_white_hsv);
    rangeColor(R_HSV, R_Y, lower_yellow_hsv, upper_yellow_hsv);
	...
	Mat ELLIPSE_5, ELLIPSE_3;
    imageMorphologyKernel(ELLIPSE_5, MORPH_ELLIPSE, 5);
    imageMorphologyKernel(ELLIPSE_3, MORPH_ELLIPSE, 3);
	imageMorphologyEx(L_WY, L_HSV, MORPH_GRADIENT, ELLIPSE_5, 1);
	imageMorphologyEx(L_HSV, L_HSV, MORPH_ERODE, ELLIPSE_3, 1);
	//imageMorphologyEx(L_HSV, L_HSV, MORPH_DILATE, ELLIPSE_3, 2);
	imageMorphologyEx(R_WY, R_HSV, MORPH_GRADIENT, ELLIPSE_5, 1);
	imageMorphologyEx(R_HSV, R_HSV, MORPH_ERODE, ELLIPSE_3, 1);
	//imageMorphologyEx(R_HSV, R_HSV, MORPH_DILATE, ELLIPSE_3, 2);
    cannyEdge(L_HSV, L_HSV, 100, 200);
    cannyEdge(R_HSV, R_HSV, 100, 200);
    result = L_HSV+R_HSV;
    
	vector<Vec4i> lines;
	imageHoughLinesP(result, lines, 1.0, PI/180.0, 20);

	Mat roadLines;
	vector<vector<Point> > points;
	splitLines(lines, points);
	linefitting(image, result, points);
    return result;
}
int main(void) {
