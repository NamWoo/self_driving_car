...
void frameProcessing(Mat &frame, Mat &result) {
    convertColor(frame, result, COLOR_BGR2GRAY);
    
	cannyEdge(result, result, 100, 200);
    Mat ROI_L, ROI_R;
    vector<Point> pts;
	pts.push_back(Point(451,323));
	pts.push_back(Point(459,323));
	pts.push_back(Point(306,452));
	pts.push_back(Point(237,452));
	polyROI(result, ROI_L, pts);
	
	pts.clear();
	pts.push_back(Point(486,327));
	pts.push_back(Point(525,327));
	pts.push_back(Point(722,430));
	pts.push_back(Point(630,430));
	polyROI(result, ROI_R, pts);
	result = ROI_L+ROI_R;
	
	vector<Vec4i> lines;
	imageHoughLinesP(result, lines, 1.0, PI/180.0, 40);
	result = imageCopy(frame);
	drawHoughLinesP(result, lines);		
    return;
}
int main(void) {
