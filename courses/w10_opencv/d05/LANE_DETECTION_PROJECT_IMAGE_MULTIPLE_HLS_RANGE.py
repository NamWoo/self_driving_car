...
def Processing(image_list, ROI_list, HLS_lists, IMSHOW):
    result_list = []
    for i in range(len(image_list)):
        inner_list = []
        for j in range(len(image_list[i])):
            inner_list.append(imageProcessing(image_list[i][j], ROI_list[i], HLS_lists[i], IMSHOW))
...
def HLS_definition():
    ...
    HLS_lists.append([lower_white_hls, upper_white_hls, lower_yellow_hls, upper_yellow_hls])
    '''
    # HLS for ETS_H_list
    lower_white_hls = np.array([0, 143, 25])
    upper_white_hls = np.array([179, 255, 255])
...
def imageProcessing(image, roi, hls_list, IMSHOW):
    result = imageCopy(image)
    height, width = result.shape[:2]
    if(IMSHOW is True):
        imageShow("image", image)
        imageShow("result", result)
    lower_white_hls = hls_list[0]
    upper_white_hls = hls_list[1]
    lower_yellow_hls = hls_list[2]
    upper_yellow_hls = hls_list[3]
...
ROIs = ROI_definition()
HLS_lists = HLS_definition()
Images = openImages()
result_list = Processing(Images, ROIs, HLS_lists, True)
