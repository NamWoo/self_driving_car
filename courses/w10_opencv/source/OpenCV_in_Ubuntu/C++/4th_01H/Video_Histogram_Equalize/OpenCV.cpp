...
void frameProcessing(Mat &frame, Mat &result) {
    ...
    //
	convertColor(result1, result1, COLOR_BGR2HSV);
    
    //
    vector<Mat> channels;
	splitImage(result1, channels);
	Mat h, s, v;
	h = channels[0];
	s = channels[1];
	v = channels[2];
    ...
    //
    histogramEqualize(s, s);
    histogramEqualize(s, v);

    // histogramEqualize(v, roadGrayHist);

    mergeImage(b, g, r, merged);
