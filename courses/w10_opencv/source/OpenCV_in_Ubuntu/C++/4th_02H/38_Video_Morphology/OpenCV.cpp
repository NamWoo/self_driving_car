...
void frameProcessing(Mat &frame, Mat &result) {
    ...
    imageDilation(result, roadGray_Dilation, kernel, 1);
    imageErosion(result, roadGray_Erosion, kernel, 1);
    imageOpening(result, roadGray_Opening, kernel, 1);
    imageClosing(result, roadGray_Closing, kernel, 1);
    imageMorphologicalGradient(result, result);
    // convertColor(result, result, COLOR_BGR2GRAY);
	// imageThreshold(result, result, 30, 255);

    // Mat CROSS;
    // imageMorphologyKernel(CROSS, MORPH_CROSS);
    // imageMorphologyEx(result, result, );
