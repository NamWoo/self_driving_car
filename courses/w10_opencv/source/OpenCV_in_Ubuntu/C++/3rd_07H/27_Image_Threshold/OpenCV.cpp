...
int main(void) {
	...
	for ( i = 0 ; i < 512 ; i++) {
	    ...
	    for ( j = 256 ; j < 512 ; j++ ) {
	        setPixel(img, i, j, 256-j);
	    }
	}
    imageShow("img", img);
    
    Mat img_THRESH_BINARY, img_THRESH_BINARY_INV, img_THRESH_TRUNC, img_THRESH_TOZERO, img_THRESH_TOZERO_INV;
    
    imageThreshold(img, img_THRESH_BINARY, 128, 255, THRESH_BINARY);
    imageThreshold(img, img_THRESH_BINARY_INV, 128, 255, THRESH_BINARY_INV);
    imageThreshold(img, img_THRESH_TRUNC, 128, 255, THRESH_TRUNC);
    imageThreshold(img, img_THRESH_TOZERO, 128, 255, THRESH_TOZERO);
    imageThreshold(img, img_THRESH_TOZERO_INV, 128, 255, THRESH_TOZERO_INV);
