...
void frameProcessing(Mat &frame, Mat &result) {
    ...
    points_01.push_back(Point(int(width * 0.35), int(height * 0.65)));
    points_01.push_back(Point(int(width * 0.65), int(height * 0.65)));
    points_01.push_back(Point(width, height));
    Mat roadColor_polyROI_01, result1;
    polyROI(result, result1, points_01);
    ...
    splitColor(result1, output_white_hls, lower_white_hls, upper_white_hls, COLOR_BGR2HLS);
    splitColor(result1, output_yellow_hls, lower_yellow_hls, upper_yellow_hls, COLOR_BGR2HLS);
    //Mat result = output_white_hls+output_yellow_hls;
    result1 = output_white_hls+output_yellow_hls;
    ...
    points_01.push_back(Point(int(width * 0.65), int(height * 0.65)));
    points_01.push_back(Point(width, height));
    points_01.push_back(Point(width, 0));
    points_01.push_back(Point(0, 0));
    points_01.push_back(Point(0, height));
    polyROI(result, ROI2, points_01);
    // addImage(result, result1, result);

    // Mat roadSum_01, roadSum_02;
    addWeightedImage(result1, ROI2, result, 100.0, 100.0);

    // imageMedianBlur(result, result, 20);
