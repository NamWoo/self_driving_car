...
int main(void) {
    ...
    // int width = roadColor.rows;
    // int height = roadColor.cols;
	int i, j;
	for ( i = 0 ; i < roadColor.cols ; i++) {
	    for ( j = 0 ; j < roadColor.rows; j++) {
	        setPixel(l2, i, j, 200);
	        // setPixel(l2, i, j, 200, 1);
	        // setPixel(l2, i, j, 200, 2);
	    }
	}
    ...
    Mat merged;
    mergeImage(h2, l2, s2, merged);
    imageShow("merged", merged);
    ...
    Mat roadBGR;
    convertColor(merged, roadBGR, COLOR_HLS2BGR);
    imageShow("roadHLS", roadBGR);
