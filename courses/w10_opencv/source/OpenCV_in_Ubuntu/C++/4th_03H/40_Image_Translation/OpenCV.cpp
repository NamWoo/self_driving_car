...
int main(void) {
	...
	Mat road_Translation_01, road_Translation_02, road_Translation_03;
	
	int width = roadColor.cols;
	int height = roadColor.rows;
	int dx = 100;
	int dy = 200;
	Size dsize(width+dx, height+dy);
	imageTranslation(roadColor, road_Translation_01, dsize, dx, dy);
	imageTranslation(road_Translation_01, road_Translation_02, Size(width, height), -dx, -dy);
	imageTranslation(road_Translation_01, road_Translation_03, Size(width, height), dx, dy, WARP_INVERSE_MAP);
	
	imageShow("road_Translation_01", road_Translation_01);
	imageShow("road_Translation_02", road_Translation_02);
	imageShow("road_Translation_03", road_Translation_03);
    destroyAllWindows();
    return 0;
}
