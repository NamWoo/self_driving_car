...
int main(void) {
    string openPath = imagepath + test;
	
	Mat roadColor = imageRead(openPath, IMREAD_COLOR);
	imageShow("roadColor", roadColor);
	
	Mat roadAffine_01, roadAffine_02, roadAffine_03;
	
	int width = roadColor.cols;
	int height = roadColor.rows;
	
	vector<Point> src_pts, dst_pts;
	src_pts.push_back(Point(int(885), int(440)));
	src_pts.push_back(Point(int(910), int(440)));
	src_pts.push_back(Point(int(910), int(460)));
	src_pts.push_back(Point(int(885), int(460)));

	dst_pts.push_back(Point(int(width*0.4), int(height*0.4)));
	dst_pts.push_back(Point(int(width*0.6), int(height*0.4)));
	dst_pts.push_back(Point(int(width*0.6), int(height*0.6)));
	dst_pts.push_back(Point(int(width*0.4), int(height*0.6)));
	
	Mat roadPoint;
	drawCircle(roadColor, roadPoint, src_pts[0], 5, Scalar(255, 0, 0), -1);
	drawCircle(roadPoint, roadPoint, src_pts[1], 5, Scalar(0, 255, 0), -1);
	drawCircle(roadPoint, roadPoint, src_pts[2], 5, Scalar(0, 0, 255), -1);
	drawCircle(roadPoint, roadPoint, src_pts[3], 5, Scalar(255, 0, 255), -1);
	imageShow("roadPoint", roadPoint);
	
	imagePerspectiveTransformation(roadPoint, roadAffine_01, src_pts, dst_pts, Size());
	// imagePerspectiveTransformation(roadAffine_01, roadAffine_02, dst_pts, src_pts, Size());
	// imagePerspectiveTransformation(roadAffine_01, roadAffine_03, src_pts, dst_pts, Size(), WARP_INVERSE_MAP);	
