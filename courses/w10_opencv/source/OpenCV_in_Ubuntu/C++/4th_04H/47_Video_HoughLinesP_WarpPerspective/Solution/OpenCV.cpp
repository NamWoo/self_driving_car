...
void frameProcessing(Mat &frame, Mat &result) {
	...
	src_pts.push_back(Point(int(width*0.55), int(height*0.65)));
	...
	imagePerspectiveTransformation(edge, roi, src_pts, dst_pts, Size());	
	vector<Vec4i> lines;
	imageHoughLinesP(roi, lines, 1.0, PI/180.0, 40);
	color = Mat(frame.rows, frame.cols, CV_8UC3, Scalar(0,0,0));
	drawHoughLinesP(color, lines);	
	imagePerspectiveTransformation(color, color, src_pts, dst_pts, Size(), WARP_INVERSE_MAP);	
	result = frame + color;
    return;
}
