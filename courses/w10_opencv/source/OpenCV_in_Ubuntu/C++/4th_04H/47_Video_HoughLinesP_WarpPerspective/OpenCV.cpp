...
void frameProcessing(Mat &frame, Mat &result) {
    ...
    namedWindow("window1", WINDOW_GUI_EXPANDED);
    namedWindow("window2", WINDOW_GUI_EXPANDED);
    namedWindow("window3", WINDOW_GUI_EXPANDED);
    ...
    // pts.push_back(Point(int(425), int(327)));
	// pts.push_back(Point(int(539), int(327)));
	pts.push_back(Point(width, height));    //right conner
	// pts.push_back(Point(810, 458));    //right conner
    pts.push_back(Point(int(width*0.7), height));
	pts.push_back(Point(int(width*0.55), int(height*0.65)));
	pts.push_back(Point(int(width*0.45), int(height*0.65)));
    pts.push_back(Point(int(width*0.3), height));
	pts.push_back(Point(0, height));        //left conner
    // pts.push_back(Point(196, 458));        //left conner
	polyROI(result, roadROI, pts);


    vector<Vec4i> lines;
	imageHoughLinesP(roadROI, lines, 1.0, PI/180.0, 100);
	Mat coppty = imageCopy(frame);
	drawHoughLinesP(coppty, lines);
    result = coppty;


    // /*image perspective Transformation*/
    // vector<Point> src_pts, dst_pts;
	// src_pts.push_back(Point(int(width * 0.35), int(height * 0.65)));
	...
	// dst_pts.push_back(Point(int(width*0), int(0)));
	// dst_pts.push_back(Point(int(width*0.9), int(0)));
	// dst_pts.push_back(Point(int(width*0.9), int(height)));
	// dst_pts.push_back(Point(int(width*0), int(height)));
	
	// drawCircle(result, result, src_pts[0], 5, Scalar(255, 0, 0), -1);
	// drawCircle(result, result, src_pts[1], 5, Scalar(0, 255, 0), -1);
	// drawCircle(result, result, src_pts[2], 5, Scalar(0, 0, 255), -1);
	// drawCircle(result, result, src_pts[3], 5, Scalar(255, 0, 255), -1);
	// imagePerspectiveTransformation(result, result, src_pts, dst_pts, Size());




    // result = roadAffine_01;
    ...
    // // result = roadROI;
    // result = imageCopy(coppty);
    
    //solution
        // cannyEdge(roadROI, roadROI, 100, 200);
        
        // vector<Vec4i> lines;
        // imageHoughLinesP(roadROI, roadROI, 1.0, PI/180.0, 50);

        // Mat perspective_lines;
        
        // convertColor(result, perspective_lines, COLOR_GRAY2BGR);	
        // drawHoughLinesP(perspective_lines, lines);	
        // imagePerspectiveTransformation(perspective_lines, result, dst_pts, src_pts, Size());
        
        // convertColor(result, perspective_lines);	
        // imageThreshold(perspective_lines, perspective_lines, 1, 255, THRESH_BINARY);
        // Mat temp;
        
        // convertColor(perspective_lines, perspective_lines, COLOR_GRAY2BGR);	
        // temp = frame - perspective_lines;
        // result = temp + result;
	...
	// vector<Point> src_pts, dst_pts;
	// src_pts.push_back(Point(425,327));
	// src_pts.push_back(Point(539,327));
	// src_pts.push_back(Point(810,458));
	// src_pts.push_back(Point(196,458));
	// dst_pts.push_back(Point(int(width*0.1), 0));
	// dst_pts.push_back(Point(int(width*0.9), 0));
	// dst_pts.push_back(Point(int(width*0.9), height));
	// dst_pts.push_back(Point(int(width*0.1), height));
	// imagePerspectiveTransformation(result, result, src_pts, dst_pts, Size());
