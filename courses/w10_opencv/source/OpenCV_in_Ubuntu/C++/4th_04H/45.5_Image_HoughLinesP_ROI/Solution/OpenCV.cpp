...
int main(void) {
	...
	Mat roadGray;
    convertColor(roadColor, roadGray);
	imageShow("roadGray", roadGray);
    
    Mat roadEdge;
	cannyEdge(roadGray, roadEdge, 100, 200);
	...
	polyROI(roadEdge, roadROI, pts);
	imageShow("roadROI", roadROI);
	
	vector<Vec4i> lines;
	imageHoughLinesP(roadROI, lines, 1.0, PI/180.0, 40);
	Mat roadLines = imageCopy(roadColor);
	drawHoughLinesP(roadLines, lines);		
...
void frameProcessing(Mat &frame, Mat &result) {
	...
	pts.push_back(Point(width, height));
	pts.push_back(Point(0, height));
	polyROI(result, result, pts);
	
	vector<Vec4i> lines;
	imageHoughLinesP(result, lines, 1.0, PI/180.0, 40);
	Mat color = imageCopy(frame);
