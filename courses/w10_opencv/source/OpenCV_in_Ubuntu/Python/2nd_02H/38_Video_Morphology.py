...
def frameProcessing(frame):
    ...
    gradient = imageMorphologyEx(result, cv2.MORPH_GRADIENT, kernel)
    gray = convertColor(gradient, cv2.COLOR_BGR2GRAY)
    thresh = imageThreshold(gray,30,255,cv2.THRESH_BINARY)
    # gray = convertColor(frame, cv2.COLOR_BAYER_BG2GRAY)
    # result = cv2.Canny(gray,100, 200)

    # result_Threshold = imageThreshold(roadGray_Close_CROSS, 30, 255, cv2.THRESH_BINARY)
