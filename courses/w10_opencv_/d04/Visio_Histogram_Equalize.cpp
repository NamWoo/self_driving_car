...
void frameProcessing(Mat &frame, Mat &result) {
	...
	pts.push_back(Point(int(width), int(height)));
	pts.push_back(Point(int(0), int(height)));
	
	Mat ROI, ROI2;
	polyROI(frame, ROI, pts);
	ROI2 = frame - ROI;
	
	convertColor(ROI, ROI, COLOR_BGR2HSV);
	vector<Mat> channels;
	splitImage(ROI, channels);
	histogramEqualize(channels[1], channels[1]);
	histogramEqualize(channels[2], channels[2]);
    mergeImage(channels, ROI);
    
    convertColor(ROI, ROI, COLOR_HSV2BGR);
	
	Scalar lower_white_hls(0, 245, 0);
	Scalar upper_white_hls(179, 255, 255);
	Scalar lower_yellow_hls(15, 75, 150);
	Scalar upper_yellow_hls(35, 175, 255);
