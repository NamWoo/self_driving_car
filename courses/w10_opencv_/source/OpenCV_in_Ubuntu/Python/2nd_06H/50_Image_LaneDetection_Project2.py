...
def splitColor(image, lower, upper, flag=None):
    ...
    mask = imageMorphologyEx(mask, cv2.MORPH_CLOSE , MORPH_ELLIPSE)
...
def imageProcessing(image):
    ...
    roadEdge = cannyEdge(output, 150, 200)
    houghLines_01 = houghLinesP(roadEdge, 1, np.pi/180, 50)
    lx, ly, rx, ry = splitLines(houghLines_01)
    draw_houghLines_01 = lineFitting(image, lx, ly, rx, ry)
        
#    imageShow("color", output)
