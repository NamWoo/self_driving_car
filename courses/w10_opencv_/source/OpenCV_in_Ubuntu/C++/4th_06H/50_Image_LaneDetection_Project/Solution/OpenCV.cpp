...
Mat imageProcessing(Mat &image) {
    ...
    vector<Point> pts;
	pts.push_back(Point(int(width * 0.4), int(height * 0.6)));
	pts.push_back(Point(int(width * 0.6), int(height * 0.6)));
	pts.push_back(Point(width, height));
	pts.push_back(Point(0, height));
	polyROI(image, result, pts);
	...
	vector<Vec4i> lines;
    imageHoughLinesP(result, lines, 1.0, PI/180.0, 50, 10);
