...
int main(void) {
	...
	Point pt1 = Point(0, int(height * 0.5));
	Point pt2 = Point(width, height);
	Mat roi_road;
	CutRectROI(roadColor, roi_road, pt1, pt2);
	imageShow("roi_road", roi_road);
	
	Mat roadColor2 = imageCopy(roadColor);
	Point new_pt1 = Point(0, 0);
	Point new_pt2 = Point(roi_road.cols, roi_road.rows);
	PasteRectROI(roi_road, roadColor2, new_pt1, new_pt2);
	imageShow("roadColor2", roadColor2);
	
	vector<Point> points_01;
	points_01.push_back(Point(0, height));
	points_01.push_back(Point(int(width * 0.5), int(height * 0.5)));
	points_01.push_back(Point(width, height));
	Mat roadColor_polyROI_01;
	polyROI(roadColor, roadColor_polyROI_01, points_01);
	imageShow("roadColor_polyROI_01", roadColor_polyROI_01);
	
	vector<Point> points_02;
	points_02.push_back(Point(int(width * 0.4), int(height * 0.6)));
	points_02.push_back(Point(int(width * 0.6), int(height * 0.6)));
	points_02.push_back(Point(width, height));
	points_02.push_back(Point(0, height));
	Mat roadColor_polyROI_02;
	polyROI(roadColor, roadColor_polyROI_02, points_02);
