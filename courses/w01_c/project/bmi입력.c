...
void chk_bmi(double check_bmi)
{
	if (check_bmi < 18.5) printf("밥은 .. 먹고 다니나요?...좀 더 드세요...\n\n\n");
	else if (check_bmi < 24.9) printf("정상이네? 지금 유지 안하면 그래도 훅가는거 알죠?(18.5 - 24.9)\n\n\n");
	else if (check_bmi < 29.9) printf("과체중... 늦었을 땐 이미 늦은거에요. 당장 운동 시작하세요 (25 - 29.9)\n\n\n");
	else if (check_bmi < 34.9) printf("경도 비만 (1단계 비만) 비만! 비이~만~~! : 30 - 34.9\n\n\n");
	else if (check_bmi < 39.9) printf("중도 비만.. 어쩌려고 그래요.. (중등 2단계 비만) (35 - 39.9)\n\n\n");
	else if (check_bmi < 40) printf("... 말 안해도 알지요? 심각한거?\n\n\n"); //(check_bmi < 40)
	else printf("키랑 몸무게 제대로 입력 한 거 맞나요 다시!?");
}
