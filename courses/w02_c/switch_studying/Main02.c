...
void Main(void)
{
	...
	Uart_Send_Byte('\n');
//	Uart_Send_Byte('A');	
	Uart_Send_String("Hello my switch test\n");
...
void my_INIT_LED(void)
{
	...
	Uart_Send_String("LED INIT done\n");
}
...
int my_GET_SW(void)
{
	...
	if ((rGPFDAT & 0x04) == 0x0)			//GPF2
	{
		sw = 4;
	}
	else if ((rGPFDAT & 0x08) == 0x0)	//GPF3
	{
		sw = 5;
	}
	else if ((rGPFDAT & 0x10) == 0x0)	//GPF4
	{
		sw = 6;
	}
	else if ((rGPFDAT & 0x20) == 0x0)	//GPF5
	{
		sw = 7;
	}
	else if ((rGPFDAT & 0x80) == 0x0)	//GPF6
	{
		sw = 8;
	}
	else{}
	...
	rGPFDAT |= (0x01<<14);				//GPF7 ignore 01
	...
	rGPFDAT &= ~(0x01<<14);				//GPF7 return 00 
